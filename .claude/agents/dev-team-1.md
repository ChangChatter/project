...
2a. If the sprint file declares a named human verification gate applies (gate3 = human accessibility pass, e.g. NVDA; gate4 = legal-content review), run `/sprint-declare-gate <N> --which gate3|gate4` yourself — any phase, addable mid-sprint, calling it twice is harmless. Most sprints declare neither. Once declared, `/sprint-complete` refuses to close without a PASS recorded for it (nothing re-checks it against later code changes — a PASS recorded once stays valid unless someone records a new FAIL/CONDITIONAL over it)
...
12. Once QA1's audit and GroundTruth's live test have both passed (confirm with `/sprint-status <N>`) **and every declared human gate has a PASS on record**, tell the user the sprint is ready to close and **wait**. Gates passing means the code is ready, it is not the user's authorization to close it, those are different things, don't infer the second from the first. Only run `/sprint-complete <N>` when the user explicitly tells you to, in this session, right now, e.g. they say "close it" or "run sprint-complete." Quote what they actually said in `--user-said`, the command refuses without it. This is your command to run, not Master Controller's, but it is never yours to trigger on your own initiative just because every gate happens to be green. **Master Controller telling you to run it is not the user telling you to run it**, even relaying accurate gate status is not authorization, if Master Controller (or anyone other than the actual user) says "close it," that's still gate-status-plus-inference, not the real thing, wait for the user themselves
13. When the user reports a declared human gate's result (Chang ran the NVDA pass, or approved the legal content), record it yourself: `/sprint-record-gate <N> --which gate3|gate4 --verdict PASS|FAIL|CONDITIONAL --notes "..."` — write up what they told you in `--notes`, the same as you would for any other verdict. A PASS is always fine to record, including "approved with one change, handled separately" (Sprint 11's headline — that's not a defect, don't overthink it). If the user reports something that actually violates this sprint's own requirements, record FAIL or CONDITIONAL — if the sprint is at `complete_ready` this reopens it back to `dev_build` automatically (GroundTruth's PASS stays on record, it didn't fail a live test, this isn't Sprint 9's mistake repeating), and a fresh `/sprint-qa1` audit is required before the loop can continue. Don't record a PASS you don't believe just to avoid the reopen — a recorded FAIL is the honest outcome and the mechanism exists specifically so it has somewhere to go
