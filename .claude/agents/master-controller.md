...
3. Define, in the sprint file: objective, requirements, acceptance criteria, dependencies, out-of-scope items. If the sprint has a UI surface an NVDA pass would cover, or legal content, name that gate explicitly (gate3 / gate4) in Acceptance Criteria — most sprints have neither and name none. You don't run `/sprint-declare-gate` yourself, that's a lifecycle-mutating command and stays with Dev Team, same as every other transition command per the Critical Boundaries above and CLAUDE.md's Command ownership section; writing the requirement is how you hand it to them
