...
- **Deployed URL: none exists yet.** I checked, not assumed: `main` currently has no app code at all — this sprint's Next.js app lives only on branch `sprint-1-app-skeleton`, uncommitted to `main`, because Dev Team doesn't push (CLAUDE.md). Whatever Vercel would build from `main` right now would not contain this sprint's work. This is requirement 8's real, current blocker, not a formality — it resolves the moment Pipeman ships this branch to `main`, at which point Vercel's existing auto-deploy picks it up and the URL goes here.
